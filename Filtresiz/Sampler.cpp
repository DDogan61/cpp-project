...
// 1 writes every frame the stack walk produces, system DLLs included: ntdll,
// kernel32, the CRT, whatever the target was calling into. 0 keeps only the
// frames that live inside the target exe.
//
// Off by default because the chart gets noisy fast, and because the frames
// underneath a blocking call are all scaffolding: waiting inside
// NtWaitForSingleObject tells you nothing that "we are stuck in getchar" did
// not already. Turn it on when a call disappears into a DLL and you want to
...
void PrintStack(HANDLE process, HANDLE thread, DWORD threadId, DWORD elapsedMs)
{
    ...
    //
    // Roomy on purpose: with the filter off a stack carries three or four times
    // as many frames, each with a full source path. Running out mid frame only
    // truncates the frame list, never the JSON, but a truncated list is a
    // missing box on the chart.
    char line[65536];
    int n = 0;
    int w = 0;
    int appFrames = 0;
